...
def get_cards_next_cards() -> list[AnkiCard]:
    with Session(engine) as sess:
        statement = select(AnkiCard).where(AnkiCard.next_date <= date.today())

        if st.session_state.select_category != "All":
            statement = statement.where(
                AnkiCard.category == st.session_state.select_category
            )
...
if idx == len(cards):
    ...
    with left:
        ...
        if failures:
            ...
            st.markdown("<div style='color:#888;'>None!</div>", unsafe_allow_html=True)
